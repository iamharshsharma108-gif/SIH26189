...
def init_server_db():
    with db() as c:
        ...
        c.execute("CREATE TABLE IF NOT EXISTS password_reset_tokens(id INTEGER PRIMARY KEY AUTOINCREMENT, account_type TEXT NOT NULL, account_id INTEGER, email TEXT NOT NULL, token_hash TEXT NOT NULL UNIQUE, expires_at TEXT NOT NULL, used INTEGER NOT NULL DEFAULT 0, created_at TEXT NOT NULL)")
        c.execute("CREATE TABLE IF NOT EXISTS admin_credentials(id INTEGER PRIMARY KEY CHECK(id=1), email TEXT NOT NULL, password_hash TEXT NOT NULL, updated_at TEXT NOT NULL)")
...
def admin_reset_email():
    try:
        return str(st.secrets.get("admin_reset_email", "")).strip().lower()
    except Exception:
        return ""

def admin_initial_password():
    try:
...
def admin_credentials():
    emails=admin_emails()
    with db() as c:
        row=c.execute("SELECT * FROM admin_credentials WHERE id=1").fetchone()
        if row: return dict(row)
        email=admin_reset_email() or (sorted(emails)[0] if emails else "")
        initial=admin_initial_password()
        if not email or not initial: return None
        c.execute("INSERT INTO admin_credentials(id,email,password_hash,updated_at) VALUES(1,?,?,?)",(email,hash_password(initial),now_str()))
        c.commit()
        return dict(c.execute("SELECT * FROM admin_credentials WHERE id=1").fetchone())
...
def is_admin_user(user=None):
    ...
    return bool(user.get("role") == "admin" or is_admin_email(user.get("email")))

def admin_login(email,password):
    email=email.strip().lower()
    if not is_admin_email(email): return None
    cred=admin_credentials()
    if cred and secrets.compare_digest(email,cred['email']) and verify_password(password,cred['password_hash']):
        return {'id':1,'email':email,'role':'admin','username':'Administrator'}
    return None

def create_reset_token(account_type, account_id, email, ttl_minutes=30):
    raw=secrets.token_urlsafe(32)
    token_hash=hashlib.sha256(raw.encode()).hexdigest()
    expires=(datetime.now()+timedelta(minutes=ttl_minutes)).strftime("%Y-%m-%d %H:%M:%S")
    with db() as c:
        c.execute("UPDATE password_reset_tokens SET used=1 WHERE account_type=? AND email=? AND used=0",(account_type,email.lower()))
        c.execute("INSERT INTO password_reset_tokens(account_type,account_id,email,token_hash,expires_at,created_at) VALUES(?,?,?,?,?,?)",(account_type,account_id,email.lower(),token_hash,expires,now_str()))
        c.commit()
    return raw,expires

def app_base_url():
    try: return str(st.secrets.get("app_base_url", "")).strip().rstrip("/")
    except Exception: return ""

def send_password_reset(account_type, identifier):
    email=identifier.strip().lower()
    if account_type=="user":
        with db() as c: row=c.execute("SELECT id,username,email FROM users WHERE lower(email)=?",(email,)).fetchone()
        if not row: return False,"If that email is registered, a reset link has been sent."
        account_id=row['id']; target=email
    else:
        if not is_admin_email(email): return False,"If that admin email is configured, a reset link has been sent."
        cred=admin_credentials()
        if not cred or cred['email']!=email: return False,"If that admin email is configured, a reset link has been sent."
        account_id=1; target=email
    raw,expires=create_reset_token(account_type,account_id,target)
    base=app_base_url()
    if not base: return False,"Password reset email is not fully configured. Add app_base_url to server secrets."
    link=f"{base}/?reset_token={raw}&account_type={account_type}"
    subject=f"Cyber Rakshak · {account_type.title()} password reset"
    body=(f"Cyber Rakshak password reset\n\nUse this secure link to choose a new password:\n{link}\n\n"
          f"This link expires at {expires} and can be used only once.\n\nIf you did not request this, ignore this email.")
    return send_case_email(target,subject,body)

def reset_password_from_token(token, account_type, new_password):
    if len(new_password)<8: return False,"Use a password of at least 8 characters."
    token_hash=hashlib.sha256(token.encode()).hexdigest()
    with db() as c:
        row=c.execute("SELECT * FROM password_reset_tokens WHERE token_hash=? AND account_type=? AND used=0",(token_hash,account_type)).fetchone()
        if not row: return False,"This reset link is invalid or has already been used."
        if datetime.strptime(row['expires_at'],"%Y-%m-%d %H:%M:%S") < datetime.now(): return False,"This reset link has expired. Request a new one."
        if account_type=="user":
            c.execute("UPDATE users SET password_hash=?,updated_at=? WHERE id=?",(hash_password(new_password),now_str(),row['account_id']))
        else:
            c.execute("UPDATE admin_credentials SET password_hash=?,updated_at=? WHERE id=1",(hash_password(new_password),now_str()))
        c.execute("UPDATE password_reset_tokens SET used=1 WHERE id=?",(row['id'],)); c.commit()
    return True,"Password changed successfully."

def password_reset_screen():
    token=st.query_params.get("reset_token","")
    account_type=st.query_params.get("account_type","user")
    if not token: return False
    st.markdown('<div class="auth-title">Set a new password</div><div class="auth-sub">Create a new password for your Cyber Rakshak account.</div>',unsafe_allow_html=True)
    with st.form("reset_password_form"):
        pw=st.text_input("New password",type="password",placeholder="Minimum 8 characters")
        confirm=st.text_input("Confirm new password",type="password")
        ok=st.form_submit_button("CHANGE PASSWORD",type="primary",width="stretch")
    if ok:
        if pw!=confirm: st.error("Passwords do not match.")
        else:
            good,msg=reset_password_from_token(token,account_type,pw)
            if good:
                st.success(msg+" You can now return to the login screen.")
...
def admin_dashboard():
    if not is_admin_user():
        st.error("Admin access is not enabled for this account."); return
    st.markdown('<div class="workspace-head"><div class="section-kicker">SERVER ADMIN</div><div class="workspace-title">Central response console.</div><div class="workspace-sub">Authorized server-side view of registrations, activity, feedback, cases, evidence and dataset provenance.</div></div>',unsafe_allow_html=True)
    with st.expander("🔐 Admin password & security", expanded=False):
        st.caption("Change the administrator password after signing in. A reset link is available from the ADMIN login screen.")
        with st.form("admin_change_password_form"):
            ...
            confirm_pw=st.text_input("Confirm new admin password",type="password")
            change=st.form_submit_button("CHANGE ADMIN PASSWORD",type="primary")
        if change:
            cred=admin_credentials()
            if not cred or not verify_password(current,cred['password_hash']): st.error("Current admin password is incorrect.")
            elif len(new_pw)<8: st.error("Use a password of at least 8 characters.")
            elif new_pw!=confirm_pw: st.error("Passwords do not match.")
            else:
                with db() as c:
                    c.execute("UPDATE admin_credentials SET password_hash=?,updated_at=? WHERE id=1",(hash_password(new_pw),now_str())); c.commit()
    ...
    with db() as c:
        ...
        logins=[dict(r) for r in c.execute("SELECT id,user_id,identifier,method,status,created_at FROM login_activity ORDER BY created_at DESC").fetchall()]
        ...
        evidence=[dict(r) for r in c.execute("SELECT id,case_id,ev_code,type,source,raw,analysis_json,timestamp FROM evidence ORDER BY id DESC").fetchall()]
        datasets=[dict(r) for r in c.execute("SELECT id,user_id,case_id,dataset,source,records,columns,evidence_hash,created_at FROM dataset_log ORDER BY created_at DESC").fetchall()]
    ...
    cols=st.columns(7)
    for col,(label,val) in zip(cols,[("Users",len(users)),("Logins",len(logins)),("Cases",len(cases)),("Evidence",len(evidence)),("Datasets",len(datasets)),("Feedback",len(feedback)),("Contacts",len(contacts))]): col.metric(label,val)
    tabs=st.tabs(["Registrations","Login Activity","Feedback","Contact","Cases","Evidence","Dataset Activity"])
    data={"Registrations":users,"Login Activity":logins,"Feedback":feedback,"Contact":contacts,"Cases":cases,"Evidence":evidence,"Dataset Activity":datasets}
    for tab,name in zip(tabs,data):
        with tab:
            df=pd.DataFrame(data[name])
            if df.empty: st.info("No records available yet.")
            else:
                search=st.text_input("Search this section",key=f"admin_search_{name}")
                view=df
                if search.strip():
                    mask=view.astype(str).apply(lambda row: row.str.contains(search.strip(),case=False,na=False).any(),axis=1); view=view[mask]
                st.dataframe(view,width="stretch",hide_index=True)
                filename=f"cyber_rakshak_{name.lower().replace(' ','_')}.csv"
                st.download_button("EXPORT CSV",view.to_csv(index=False).encode("utf-8"),filename,"text/csv",width="stretch",key=f"admin_export_{name}")
...
def auth_screen():
    ...
    st.markdown("<div class='auth-title'>Cyber Rakshak</div><div class='auth-sub'>Secure access · choose your account type</div>",unsafe_allow_html=True)
    if password_reset_screen():
        st.markdown("</div>",unsafe_allow_html=True); return
    role=st.radio("Account type",["USER","ADMIN"],horizontal=True,key="auth_role_choice")
    if role=="USER":
        mode=st.radio("User access",[t('login'),t('register')],horizontal=True,key="user_auth_mode_choice")
        if mode==t('login'):
            ...
            if ok:
                ...
                if user:
                    ...
                    if cases:
                        st.session_state.case={k:cases[0].get(k,'') for k in ['id','title','subject','aliases','description','priority','created']}; st.session_state.notes=cases[0].get('notes','') or ''; st.session_state.evidence=load_evidence(st.session_state.case['id'])
                    else: st.session_state.case=default_case(); st.session_state.evidence=[]
                    st.session_state.nav='Home'; st.rerun()
                else: st.error('We could not sign you in. Check your email/username and password.')
            st.markdown("<div style='text-align:center;margin:12px 0'>",unsafe_allow_html=True)
            forgot_email=st.text_input("Forgot password? Enter your registered email",key="user_forgot_email")
            if st.button("SEND RESET LINK",key="user_forgot_btn",width="stretch"):
                ok,msg=send_password_reset("user",forgot_email); st.info(msg)
            st.markdown("</div>",unsafe_allow_html=True)
            if google_auth_configured():
                if st.button('Continue with Google',key='google_login',width='stretch'): st.login()
        else:
            ...
            if ok:
                if not name.strip(): st.error('Please enter your name.')
                    ...
                    if good: st.success(msg+' You can now sign in with your email or username.')
                    else: st.error(msg)
    else:
        st.markdown("<div class='auth-title'>Admin Access</div><div class='auth-sub'>Authorized administrator only · no public admin registration</div>",unsafe_allow_html=True)
        with st.form('admin_login_form'):
            email=st.text_input('Admin email',placeholder='configured administrator email',autocomplete='username')
            password=st.text_input('Admin password',type='password',autocomplete='current-password')
            ok=st.form_submit_button('LOGIN AS ADMIN',type='primary',width="stretch")
        if ok:
            admin=admin_login(email,password)
            if admin:
                st.session_state.auth_user=admin; st.session_state.nav='Admin Console'; st.session_state._server_loaded=True; st.rerun()
            else: st.error('Invalid administrator credentials.')
        forgot_email=st.text_input("Forgot admin password? Enter the configured admin email",key="admin_forgot_email")
        if st.button("SEND ADMIN RESET LINK",key="admin_forgot_btn",width="stretch"):
            ok,msg=send_password_reset("admin",forgot_email); st.info(msg)
    st.markdown("<div class='auth-security'>🔒 Passwords are stored as salted PBKDF2-SHA256 hashes. Reset links are single-use and time-limited.</div></div>",unsafe_allow_html=True)
