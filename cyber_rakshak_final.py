...
def analyze_transactions(df):
    # Accept both the original Part-1 schema (Sender/Receiver/Amount/Timestamp)
    # and the normalized/lowercase exports used by the bundled quick demo.
    w=df.copy(); inferred=infer_columns(w)
    def first_of(*types):
        return next((c for c,t in inferred.items() if t in types), None)
    sender_col=first_of("sender") or ("Sender" if "Sender" in w.columns else None)
    receiver_col=first_of("receiver") or ("Receiver" if "Receiver" in w.columns else None)
    amount_col=first_of("amount") or ("Amount" if "Amount" in w.columns else None)
    timestamp_col=first_of("timestamp") or ("Timestamp" if "Timestamp" in w.columns else None)
    missing=[]
    if not sender_col: missing.append("Sender")
    if not receiver_col: missing.append("Receiver")
    if not amount_col: missing.append("Amount")
    if not timestamp_col: missing.append("Timestamp")
    if missing: return {"score":0,"strength":"Weak","indicators":[f"Missing columns: {', '.join(missing)}"],"entities":[],"explanation":"Supply sender, receiver, amount and timestamp fields.","features":{}}
    w=w.rename(columns={sender_col:"Sender",receiver_col:"Receiver",amount_col:"Amount",timestamp_col:"Timestamp"})
    w["Amount"]=pd.to_numeric(w["Amount"],errors="coerce"); w=w.dropna(subset=["Amount"])
...
# Part 2 display labels. Keep this independent from the navigation labels so
# language changes never break the Network Intelligence banner.
part2_map = {
    "en": "NETWORK INTELLIGENCE",
    "hi": "नेटवर्क इंटेलिजेंस",
    "ta": "நெட்வொர்க் நுண்ணறிவு",
    "bn": "নেটওয়ার্ক ইন্টেলিজেন্স",
}
...
def auth_screen():
    ...
    with st.container(key="auth_card"):
        ...
        # Do not persist the radio widget under a session_state key. Streamlit forbids
        # modifying a widget-owned key after the widget has been instantiated. The
        # application-level auth_mode is the single source of truth instead.
        selected_mode=st.radio("Authentication", mode_options, index=mode_options.index(current_mode), horizontal=True, label_visibility="collapsed")
            ...
            if ok:
                if not name.strip(): st.error('Please enter your name.')
                    ...
                    if good:
                        ...
                        # The auth radio has no widget-owned session key, so the
                        # next rerun naturally opens on the Login option.
                        st.rerun()
                    else: st.error(msg)
