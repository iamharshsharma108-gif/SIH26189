...
def google_auth_configured():
    """Return True when Streamlit Google/OIDC secrets are configured."""
    try:
...
def init_state_unified():
    ensure_state()
    defaults={'auth_user':None,'auth_mode':'login','public_nav':'Home','case':default_case(),'case_notes':'','datasets':{},'ledger':[],'loaded_demo':False,'evidence':[],'analysis_run':0,'selected_ev':None,'lang':'en','notes':'','keep_session_data':True,'session_saved_at':None}
...
def public_landing():
    ...
    with top_signin:
        if st.button('Sign in', key='public_signin', type='primary', width='stretch'):
            st.session_state.auth_mode='login'; st.session_state.public_nav='auth'; st.rerun()
    with top_register:
        if st.button('Register', key='public_register', width='stretch'):
            st.session_state.auth_mode='register'; st.session_state.public_nav='auth'; st.rerun()
    st.markdown('<div class="top-auth-card"><div class="top-auth-label">ACCESS WORKSPACE</div><div class="top-auth-copy">Use the buttons above to enter or create your investigation workspace.</div></div>', unsafe_allow_html=True)
    st.markdown('<div class="hero-paper"><div class="eyebrow">INVESTIGATION INTELLIGENCE</div><h1>Find the connections.<br>Keep the evidence visible.</h1><p>Cyber Rakshak brings authorized records into one explainable workspace, helping analysts trace identifiers, inspect networks, compare timelines, and preserve provenance.</p></div>', unsafe_allow_html=True)
    st.markdown('<div class="hero-panel"><div class="panel-copy"><div class="panel-kicker">A CLEARER INVESTIGATION LOOP</div><div class="panel-title">Bring records together, then ask better questions.</div><div class="panel-note">Every match carries a source. Every relationship carries context. Analytical signals stay separate from observed evidence.</div></div><div class="signal-board"><div class="signal-node n1">PHONE</div><div class="signal-node n2">ACCOUNT</div><div class="signal-node n3">DOMAIN</div><div class="signal-node n4">INCIDENT</div><div class="signal-line l1"></div><div class="signal-line l2"></div><div class="signal-line l3"></div><div class="signal-center">TRACE</div></div></div>', unsafe_allow_html=True)

    st.markdown('<div id="capabilities" class="section-paper"><div class="section-kicker">WORKSPACE MODULES</div><h2>One evidence trail, several ways to inspect it.</h2><div class="cap-grid"><div class="cap"><div class="cap-no">A1</div><h3>Match & provenance</h3><p>Search phones, emails, accounts, IPs and domains while retaining dataset and record context.</p></div><div class="cap"><div class="cap-no">A2</div><h3>Network analysis</h3><p>Move from direct relationships to paths, clusters, bridges and transaction routes.</p></div><div class="cap"><div class="cap-no">A3</div><h3>Temporal review</h3><p>Put timestamped observations into sequence and compare events across sources.</p></div><div class="cap"><div class="cap-no">A4</div><h3>Evidence reporting</h3><p>Keep an auditable ledger and generate a concise case brief for authorized recipients.</p></div></div></div>', unsafe_allow_html=True)
...
def auth_screen():
    ...
    with st.container(key="auth_card"):
        ...
        with tabs[0]:
            st.markdown("<div class='auth-title'>Welcome back</div><div class='auth-sub'>Sign in to continue to your Cyber Rakshak workspace.</div>",unsafe_allow_html=True)
            with st.form('login_form'):
                identifier=st.text_input('Gmail / Email / Username',placeholder='you@gmail.com or username',autocomplete='username')
                password=st.text_input('Cyber Rakshak Password',type='password',placeholder='Enter your Cyber Rakshak password',autocomplete='current-password')
            ...
            st.markdown("<div style='display:flex;align-items:center;gap:10px;margin:14px 0 12px;color:#94a3b8;font:600 9px DM Sans,sans-serif;letter-spacing:1.4px;text-transform:uppercase;'><span style='height:1px;background:#e8edf2;flex:1'></span><span>OR</span><span style='height:1px;background:#e8edf2;flex:1'></span></div>",unsafe_allow_html=True)
            if google_auth_configured():
                ...
                st.caption('Google sign-in becomes active after the optional [auth] settings are added to .streamlit/secrets.toml.')
        ...
        with tabs[1]:
            st.markdown("<div class='auth-title'>Create account</div><div class='auth-sub'>Set up your secure investigation workspace.</div>",unsafe_allow_html=True)
            with st.form('register_form'):
                ...
                ok=st.form_submit_button('Register',type='primary',width="stretch")
            if password:
                pct,label=password_strength(password)
                st.markdown(f"<div class='strength-track'><div class='strength-fill' style='width:{pct}%;background:{'#b4233b' if pct==25 else '#0e7490' if pct==60 else '#087f5b'}'></div></div><div class='strength-text'>Password strength: <b>{label}</b></div>",unsafe_allow_html=True)
            if ok:
...
def feedback_db():
    conn=sqlite3.connect(FEEDBACK_DB_PATH); conn.row_factory=sqlite3.Row; return conn
...
def report_text_full():
    F=current_fusion(); G=build_graph('all'); case=st.session_state.case
...
with st.sidebar:
    ...
    st.markdown('<div class="sidebar-section">Workspace language</div>', unsafe_allow_html=True)
    st.selectbox('Language / भाषा / மொழி', ['English','Hindi','Tamil','Bengali'], index=['en','hi','ta','bn'].index(st.session_state.get('lang','en')), key='language_choice', label_visibility='collapsed')
    st.session_state.lang={'English':'en','Hindi':'hi','Tamil':'ta','Bengali':'bn'}.get(st.session_state.get('language_choice') or 'English','en')

    st.markdown('<div class="sidebar-section">Current case</div>', unsafe_allow_html=True)
    ...
    st.markdown('<div class="sidebar-section">Case navigation</div>', unsafe_allow_html=True)
    nav = st.session_state.nav
    for gi,(label,items) in enumerate(groups):
        current = nav if nav in items else None
        chosen = st.radio(label, items, index=(items.index(current) if current else None), key=f"nav_group_{gi}", label_visibility="visible")
        if chosen and chosen != nav:
            nav = chosen
    st.session_state.nav = nav

    st.markdown('<div class="sidebar-rule"></div>', unsafe_allow_html=True)
    st.markdown('<div class="sidebar-section">Session data</div>', unsafe_allow_html=True)
    ...
    if st.button("Clear current data", width="stretch", key="sidebar_clear"):
        st.session_state.datasets={}; st.session_state.ledger=[]; st.session_state.loaded_demo=False; st.session_state.session_saved_at=None; st.rerun()
...
if nav in PART2_NAV:
    st.markdown('<div class="part2-banner"><span>PART 2</span><b>NETWORK INTELLIGENCE</b><em>Evidence correlation · entity resolution · graph analysis</em></div>', unsafe_allow_html=True)

if nav=="Home":
    st.markdown('<div class="workspace-head"><div class="workspace-title">Evidence Intelligence Desk</div><div class="workspace-sub">A focused workspace for loading authorized records, resolving identifiers, tracing relationships and preserving the evidence trail.</div></div>',unsafe_allow_html=True)
    c1,c2,c3,c4=st.columns(4)
    for c,label,value in [(c1,"Data sources",len(all_frames())),(c2,"Records",sum(len(x) for x in all_frames().values())),(c3,"Graph nodes",build_graph("all").number_of_nodes()),(c4,"Graph relationships",build_graph("all").number_of_edges())]:
...
# ============================================================
    ...
    st.markdown('<div class="workspace-head"><div class="section-kicker">Case configuration</div><div class="workspace-title">Set the investigation context.</div><div class="workspace-sub">Case metadata is linked to your signed-in account and can be saved to the local server database.</div></div>',unsafe_allow_html=True)
    with st.form("case_setup_form"):
        title=st.text_input("Case title",value=st.session_state.case.get("title", ""),placeholder="e.g. Cross-source identifier review")
        subject=st.text_input("Primary subject / identifier",value=st.session_state.case.get("subject", ""),placeholder="Optional — phone, account, domain, incident…")
        aliases=st.text_input("Known aliases / identifiers",value=st.session_state.case.get("aliases", ""),placeholder="Optional alternate identifiers")
        priority=st.selectbox("Priority",["Low","Medium","High","Critical"],index=["Low","Medium","High","Critical"].index(st.session_state.case.get("priority","Medium")))
        description=st.text_area("Investigation note",value=st.session_state.case.get("description", ""),placeholder="What question are you trying to answer?",height=130)
        notes=st.text_area("Analyst notes",value=st.session_state.get("notes", ""),placeholder="Record observations, provenance checks or follow-up questions.",height=100)
        submitted=st.form_submit_button("Save case context",type="primary",width="stretch")
    if submitted:
    ...
    st.markdown('<div class="notice notice-green"><b>Persistence boundary:</b> case metadata and dataset provenance are stored server-side. Raw uploaded files remain in the active Streamlit session in this prototype.</div>',unsafe_allow_html=True)
...
# ============================================================
    ...
    G=build_graph("all"); signals=network_signals(G)
    if signals:
